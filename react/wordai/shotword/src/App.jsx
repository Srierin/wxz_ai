...
function App() {
  ...
  const picPrompt = `
    ...
    返回JSON数据：
    {
      "image_discription": "图片的描述",
      "representative_word": "图片代表的一个英文单词",
      "example_sentence": "结合该单词和图片描述，给出一个简单的英文句子",
      "explaination": "结合图片对该单词的进行解释，段落以Look at the picture...开头,将段落分句，每句单独一行,解释的最后给一个日常生活有关的问句",
  ...
  const [explaination, seExplainations] = useState([]);
  ...
  const uploadImage = async (imgData) => {
    ...
    setWord(data.representative_word);
  ...
  return (
    <div className="container">
      {/* 自定义组件 子组件
      组件 组合html,css,js 沙子一样，组合起来,图片上传功能
      模块化了，复用，页面由DOM树 -》组件树 */}
      {/* props */}
      <PictureCard
